...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control& control)
{
    ...
    auto& io = ImGui::GetIO();
#if 1 
    if (Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_F)) && Editor->AreShortcutsEnabled())
    {
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(const Control& control)
{
    ...
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_F)))
